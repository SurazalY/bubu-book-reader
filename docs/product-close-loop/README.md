...
| `05_主控交接说明.md` | 已完成到哪、踩过哪些坑、硬规则与未关闭缺陷索引 | **主控接手时第一篇必读，先于 01–04** |

> 换主控（含上下文压缩后的自我接续）必须先整篇读 `05_主控交接说明.md`，并在交出前按其第 10 节追加交接记录。该文件只允许追加，不得删改或精简既有内容——对话上下文不会传给继任者，只有仓库里的文件会。

...
| D-03 三维翻页回弹 | **已按裁决隐藏 `4a5e6d6`** | 用户裁决为隐藏而非删除或修复：默认改平移、curl 分支不可达、UI 去掉偏好项，代码保留。永久债记 `docs/tech-debt.md` TD-001 |

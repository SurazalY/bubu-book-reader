...
| D-17 | **无会话僵尸租约无限续期，再次锁死阅读计时与返回详情** | **阻塞** | **已修（待主控提交）** | 第三次复发。症状与 D-11/D-15 相同：返回详情死键、时长与页码不记录。现场租约 `ced73a99` 被 Cursor 内嵌阅读器续了 58 次、从未建会话。结构性根治见第十四节 |
| D-18 | 原版模式误用封面页框当全书尺子，导致部分书被放大、其余书（含 book-001）显得糊 | 应修 | **已修（待主控提交）** | `useProtectedPdfDocument` 只 `getPage(1)`，把封面 MediaBox（约 1940×2733 pt）写入全书 `width/height`；`Reader.jsx` 用其作 `pageDesign`，scale 夹在 0.4–2。8 本封面被 0.4 地板托大，其余书按窗口缩小显得糊。内页实际约 466–472×672。**本轮只改学生阅读器选尺**（抽样第 1/2/中间/末页，宽高分别取下中位数）；**不重导 book-001、不重渲染 PDF**。文字模式仍走 `PAGE_DESIGN` 468×636。设计见 `docs/product-close-loop/design/pdf-page-design.md`。另提高 canvas outputScale，CSS 尺寸不变 |
| D-19 | 原版模式尺寸统一后全书正文发糊，且提高 canvas backing 无效 | 应修 | **已修（待真人复验）** | D-18 的另一半。**根因不是渲染管线，是字太小**：这批扫描件正文汉字字身仅 8.7–9.9 pt，而按窗口高度铺满时 `scale ≈ 1.26`，汉字只有约 11 设备像素，低于汉字可读的 12–14 下沿。实测恒等式 `汉字设备像素 = 字身pt × 舞台物理像素高 ÷ 页高pt`，dpr 被约掉，所以 backing 倍率、dpr、对开/单页、浏览器缩放全都改不动它——这解释了第二轮为何无效。1080p 屏上「整页可见」与「字清晰」数学上不可兼得。修法：把原版模式的「大」字号档从空转改成真正放大到 1.6 倍 + 纵向滚动；顶栏底栏收起时真正退出 flex 流；canvas CSS 尺寸取整。详见第十五节专项 |
...
| **D-17 无会话僵尸租约** | **已修（待主控提交与真人复验）** | 第三次复发。B1 无会话也走同一停滞阈值、B2 活跃在位者不可被抢、B3 导航有界超时。真实库已观测到 `ced73a99` version 冻结在 58、续期 409 `LEASE_REQUIRED` |
...
**「测试全绿」不等于「修好了」**（第九节教训 2 的同一条）。本条在用户于真实浏览器确认之前不得关闭。

复验步骤见交付报告。若真人确认后仍不满意清晰度，**唯一还成立的产品选项是继续加大放大倍率或引入连续缩放**，不要再加 `outputScale` 倍数——那条路已被上面的恒等式判死。

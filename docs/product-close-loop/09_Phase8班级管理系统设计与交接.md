...
**迁移编号（2026-08-18 改判）。** 现有 29 个迁移文件，目录最大号是 **045**。08/本段初稿写「T5.1 取 030、本阶段从 031 起」**是错的**：前任交接没有核 `server/db/migrations/`，`030_community_reports_delivery.sql` 早已占用 030。不改号会撞社区举报表。用户已允许 Phase 5 的 T5.1 改用 **046**。**本阶段从 047 起连续取号。** 本阶段要加多次（凭据表、入班申请表、可能调整年级语义、grants 数据迁移）。开工前先确认 **046** 已落地，不要再找 030。
